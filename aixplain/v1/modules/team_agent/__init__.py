...
class TeamAgent(Model, DeployableMixin[Agent]):
    ...
    def generate_session_id(self, history: list = None) -> str:
        ...
        try:
            ...
            if self.llm is not None:
                mp = getattr(self.llm, "model_params", None)
                if mp is not None:
                    p = mp.get_parameter("max_tokens")
                    if p is not None:
                        if p.value is not None:
                            try:
                                resolved_max_tokens = int(p.value)
                            except (TypeError, ValueError):
                                pass
                        elif p.default_values and p.default_values[0] is not None:
                            try:
                                resolved_max_tokens = int(p.default_values[0])
                            except (TypeError, ValueError):
                                pass
    ...
    def run(
        ...
        **kwargs,
        ...
        Args:
            ...
            max_tokens (Optional[int], optional): maximum number of tokens which can be generated by the agents.
                If omitted, uses the main LLM's configured ``max_tokens`` when set; otherwise 4096.
    ...
    def run_async(
        ...
        if parameters.get("max_tokens") is not None:
            resolved_max_tokens = int(parameters["max_tokens"])
