    ...
    def create(
        ...
        **kwargs,
        ...
        if isinstance(params, BaseUtilityModelParams):
            ...
            assert connector.function_type == FunctionType.INTEGRATION, (
                f"The model you are trying to connect ({connector.id}) to is not a connector."
            )

            assert authentication_schema is not None, (
                "Please provide the authentication schema to use (authentication_schema parameter)"
            )
            assert isinstance(authentication_schema, AuthenticationSchema), (
                "authentication_schema must be an instance of AuthenticationSchema"
            )
            ...
            assert response.status == ResponseStatus.SUCCESS, (
                f"Failed to connect to {connector.id} - {response.error_message}"
            )
