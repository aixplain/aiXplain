...
class ModelFactory(ModelGetterMixin, ModelListMixin):
        ...
        **kwargs,
        ...
        if isinstance(code, Callable):
            ...
            assert len(function_names) > 0, (
                "No functions found in the code. Please provide at least one function in your code."
            )
            ...
            if function_name is None and len(function_names) == 1:
                ...
                raise Exception(
                    f"Multiple functions found in the code: {function_names}. Please specify at least one function name using the function_name parameter."
                )
            elif function_name and function_name not in function_names:
                raise Exception(
                    f"Function name {function_name} not found in the code. Available functions provided by the code: {function_names}. Please specify a valid function name using the function_name parameter."
                )
    ...
    @classmethod
    def list_host_machines(cls, api_key: Optional[Text] = None, **kwargs) -> List[Dict]:
    ...
    @classmethod
    def list_gpus(cls, api_key: Optional[Text] = None, **kwargs) -> List[List[Text]]:
    ...
    @classmethod
    def list_functions(cls, verbose: Optional[bool] = False, api_key: Optional[Text] = None, **kwargs) -> List[Dict]:
