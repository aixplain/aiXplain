    ...
    def create(
        ...
        try:
            ...
            if split_labels is not None and split_rate is not None:
                assert len(split_labels) == len(split_rate), (
                    "Data Asset Onboarding Error: Make sure you set the *split_labels* and *split_rate* lists must have the same length."
                )
                split_metadata = onboard_functions.split_data(
                    paths=paths, split_labels=split_labels, split_rate=split_rate
                )
            ...
            for key, schema in [
                ...
                for i in tqdm(range(len(schema)), desc=f" Dataset's {key} onboard progress", position=0):
                    ...
                    files, data_column_idx, start_column_idx, end_column_idx, nrows = (
                        onboard_functions.process_data_files(
            ...
            assert len(dataset_payload["input"]) > 0, (
            ...
            response = onboard_functions.create_data_asset(
                payload=dataset_payload, data_asset_type="dataset", api_key=api_key
            )
