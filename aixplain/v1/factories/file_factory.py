...
class FileFactory:
    ...
    def upload(
        ...
        """
        if is_temp is False:
            assert return_download_link is False, (
    ...
    def create(
        ...
        """
        assert license is not None if is_temp is False else True, (
            "File Asset Creation Error: To upload a non-temporary file, you need to specify the `license`."
        )
        return cls.upload(
            local_path=local_path, tags=tags, license=license, is_temp=is_temp, return_download_link=is_temp
        )
