...
class APIKeyFactory:
    ...
    def get(cls, api_key: Text, **kwargs) -> APIKey:
        ...
        for api_key_obj in cls.list(**kwargs):
            if str(api_key_obj.access_key).startswith(api_key[:4]) and str(api_key_obj.access_key).endswith(
                api_key[-4:]
            ):
                return api_key_obj
    ...
    def update(cls, api_key_obj: APIKey, **kwargs) -> APIKey:
        ...
        try:
            ...
            logging.info(f"Updating API key with ID {api_key_obj.id} and new values")
            r = _request_with_retry("put", url, json=payload, headers=headers)
            resp = r.json()
        except Exception as e:
            raise Exception(f"API Key Update Error: Failed to update API key with ID {api_key_obj.id}. Error: {str(e)}")
        ...
        if 200 <= r.status_code < 300:
            ...
            raise Exception(
                f"API Key Update Error: Failed to update API key with ID {api_key_obj.id}. Error: {str(resp)}"
            )
