...
def get_paths(input_paths: List[Union[str, Path]]) -> List[Path]:
    ...
    # check CSV sizes
    for path in paths:
        assert os.path.getsize(path) <= 1e9, (
...
def build_payload_data(data: Data) -> Dict:
    ...
    if (
        data.start_column is not None
        and data.start_column > -1
        and data.end_column is not None
        and data.end_column > -1
    ):
...
def create_data_asset(payload: Dict, data_asset_type: Text = "corpus", api_key: Optional[Text] = None) -> Dict:
    ...
    if 200 <= r.status_code < 300:
        ...
        try:
            ...
            error_msg = f"Data Asset Onboarding Error: Failure on creating the {data_asset_type}. Please contact the administrators."
...
def split_data(paths: List, split_rate: List[float], split_labels: List[Text]) -> MetaData:
    ...
    for path in paths:
        ...
        dataframe[column_name] = [
            slabel for (slabel, srate) in zip(split_labels, split_rate) if srate == max(split_rate)
        ][0]
