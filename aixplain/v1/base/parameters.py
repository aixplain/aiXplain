...
class BaseParameters:
    ...
    def to_list(self) -> List[str]:
        ...
        """
        return [
            {"name": param.name, "value": param.value} for param in self.parameters.values() if param.value is not None
        ]
    ...
    def __str__(self) -> str:
        ...
        for param in self.parameters.values():
            ...
            options_str = (
                f" Options: {[opt.get('value') for opt in param.available_options]}" if param.available_options else ""
            )
            lines.append(f"  - {param.name}: {value_str} {required_str}{fixed_str}{type_str}{options_str}")
