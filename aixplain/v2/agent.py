"""Agent resource module for managing agents and their execution."""
...
class Task:
    ...
    def __post_init__(self) -> None:
        """Initialize task and convert dependencies to names."""
...
class Agent(
    ...
    def __post_init__(self) -> None:
        """Initialize agent and process subagents, tasks, and other fields."""
    ...
    def before_run(self, *args: Any, **kwargs: Unpack[AgentRunParams]) -> Optional[AgentRunResult]:
        """Validate dependencies and auto-save draft agents before running.
        ...
        Returns:
            Optional early return result, or None to continue with run.
        """
    ...
    def after_run(
        ...
        **kwargs: Unpack[AgentRunParams],
        ...
        Args:
            result: The result from the agent run or an exception if it failed.
    ...
    def run(self, *args: Any, **kwargs: Unpack[AgentRunParams]) -> AgentRunResult:
        ...
        Args:
            *args: First positional argument is treated as the query string.
    ...
    def build_save_payload(self, **kwargs: Any) -> dict:
        ...
        if self.tools:
            for tool in self.tools:
                # If tool is already a dict, use it directly (already in correct format)
                if isinstance(tool, dict):
                    converted_assets.append(tool)
                # Check for ToolableMixin instances (covers Tool and Model since they inherit from it)
                elif isinstance(tool, ToolableMixin):
                    # Convert to tool dict format
                    converted_assets.append(tool.as_tool())
                else:
                    raise ValueError(
                        f"A tool in the agent must be a Tool, Model, ToolableMixin instance, or dict. "
                        f"Got {type(tool)} instead."
                    )
