...
class Pipeline(Asset):
    ...
    def __polling(
        ...
        while not response_body["completed"] and (end - start) < timeout:
            try:
                ...
                logging.error(
                    f"Polling for Pipeline '{self.id}': polling for {name} ({poll_url}): Continue"
                )
                break
        ...
        try:
            ...
            if "data" in resp and isinstance(resp["data"], str):
                try:
                    ...
                    resp = r.json()
            logging.info(
                f"Single Poll for Pipeline '{self.id}' - Status of polling for {name} ({poll_url}): {resp}"
            )
        ...
        **kwargs,
        ...
        try:
            response = self.run_async(
                data, data_asset=data_asset, name=name, version=version, **kwargs
            )
            ...
            polling_response = self.__polling(
                poll_url, name=name, timeout=timeout, wait_time=wait_time
            )
            end = time.time()
            if isinstance(polling_response, dict):
                status = ResponseStatus(polling_response.get("status", "failed"))
                completed = polling_response.get("completed", False)
            else:
                status = polling_response.status
                completed = getattr(polling_response, "completed", False)
            if response_version == "v1":
        ...
        **kwargs,
        ...
        try:
            if 200 <= r.status_code < 300:
                ...
                logging.error(
                    f"Error in request for {name} (Pipeline ID '{self.id}') - {r.status_code}: {error}"
                )
                if response_version == "v1":
                    return {
                        "status": "failed",
    ...
    def deploy(self, api_key: Optional[Text] = None) -> None:
        ...
        assert (
            self.status == "draft"
        ), "Pipeline Deployment Error: Pipeline must be in draft status."
        assert (
            self.status != "onboarded"
        ), "Pipeline Deployment Error: Pipeline must be onboarded."

        pipeline = self.to_dict()
        self.update(
            pipeline=pipeline, save_as_asset=True, api_key=api_key, name=self.name
        )
    ...
    def from_dict(cls, data: Dict) -> "Pipeline":
        ...
        Args:
            data (Dict): A dictionary containing pipeline attributes.
        ...
        return cls(
            ...
            url=data.get("url", config.BACKEND_URL),
            supplier=data.get("supplier", "aiXplain"),
            version=data.get("version", "1.0"),
            status=data.get("status", AssetStatus.DRAFT),
            **data.get("additional_info", {}),
        )

    def to_dict(self) -> Dict:
        """
        Serialize the Pipeline object to a dictionary.

        Returns:
            Dict: A dictionary representation of the Pipeline instance.
        """
        logging.info("corect to dict")
        return {
            "id": self.id,
            "name": self.name,
