__author__ = "aiXplain"

"""
Copyright 2024 The aiXplain SDK authors

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

     http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.

Author: Lucas Pavanelli and Thiago Castro Ferreira
Date: May 16th 2024
Description:
    Agentification Class
"""
import json
import logging
import re
import time

from aixplain.utils.file_utils import _request_with_retry
from aixplain.enums.function import Function
from aixplain.enums.supplier import Supplier
from aixplain.enums.asset_status import AssetStatus
from aixplain.modules.model import Model
from aixplain.modules.agent.agent_task import AgentTask
from aixplain.modules.agent.output_format import OutputFormat
from aixplain.modules.agent.tool import Tool
from aixplain.modules.agent.response import AgentResponse, AgentResponseData
from aixplain.enums import ResponseStatus
from aixplain.modules.agent.utils import process_variables
from typing import Dict, List, Text, Optional, Union
from urllib.parse import urljoin

from aixplain.utils import config
from aixplain.exceptions import (
    AgentError,
    AgentExecutionError,
    AgentIterationLimitError,
    AgentTimeoutError,
    SupplierError,
    ValidationError,
    AuthenticationError,
    BillingError,
)
from aixplain.exceptions.error_handler import ErrorHandler


class Agent(Model):
    """Advanced AI system capable of performing tasks by leveraging specialized software tools and resources from aiXplain marketplace.

    Attributes:
        id (Text): ID of the Agent
        name (Text): Name of the Agent
        tools (List[Union[Tool, Model]]): List of tools that the Agent uses.
        description (Text, optional): description of the Agent. Defaults to "".
        instructions (Text): instructions of the Agent.
        llm_id (Text): large language model. Defaults to GPT-4o (6646261c6eb563165658bbb1).
        supplier (Text): Supplier of the Agent.
        version (Text): Version of the Agent.
        backend_url (str): URL of the backend.
        api_key (str): The TEAM API key used for authentication.
        cost (Dict, optional): model price. Defaults to None.
    """

    is_valid: bool

    def __init__(
        self,
        id: Text,
        name: Text,
        description: Text,
        instructions: Text,
        tools: List[Union[Tool, Model]] = [],
        llm_id: Text = "6646261c6eb563165658bbb1",
        api_key: Optional[Text] = config.TEAM_API_KEY,
        supplier: Union[Dict, Text, Supplier, int] = "aiXplain",
        version: Optional[Text] = None,
        cost: Optional[Dict] = None,
        status: AssetStatus = AssetStatus.DRAFT,
        tasks: List[AgentTask] = [],
        **additional_info,
    ) -> None:
        """Create an Agent with the necessary information.

        Args:
            id (Text): ID of the Agent
            name (Text): Name of the Agent
            description (Text): description of the Agent.
            instructions (Text): role of the Agent.
            tools (List[Union[Tool, Model]]): List of tools that the Agent uses.
            llm_id (Text, optional): large language model. Defaults to GPT-4o (6646261c6eb563165658bbb1).
            supplier (Text): Supplier of the Agent.
            version (Text): Version of the Agent.
            backend_url (str): URL of the backend.
            api_key (str): The TEAM API key used for authentication.
            cost (Dict, optional): model price. Defaults to None.
        """
        super().__init__(id, name, description, api_key, supplier, version, cost=cost)
        self.instructions = instructions
        self.additional_info = additional_info
        self.tools = tools
        for i, _ in enumerate(tools):
            self.tools[i].api_key = api_key
        self.llm_id = llm_id
        if isinstance(status, str):
            try:
                status = AssetStatus(status)
            except Exception:
                status = AssetStatus.DRAFT
        self.status = status
        self.tasks = tasks
        self.is_valid = True
        self.error_handler = ErrorHandler(max_retries=3)

    def _validate(self) -> None:
        """Validate the Agent."""
        from aixplain.factories.model_factory import ModelFactory

        # validate name
        assert (
            re.match(r"^[a-zA-Z0-9 \-\(\)]*$", self.name) is not None
        ), "Agent Creation Error: Agent name contains invalid characters. Only alphanumeric characters, spaces, hyphens, and brackets are allowed."

        try:
            llm = ModelFactory.get(self.llm_id, api_key=self.api_key)
        except Exception:
            raise Exception(f"Large Language Model with ID '{self.llm_id}' not found.")

        assert llm.function == Function.TEXT_GENERATION, "Large Language Model must be a text generation model."

        for tool in self.tools:
            if isinstance(tool, Tool):
                tool.validate()
            elif isinstance(tool, Model):
                assert not isinstance(tool, Agent), "Agent cannot contain another Agent."

    def validate(self, raise_exception: bool = False) -> bool:
        """Validate the Agent."""
        try:
            self._validate()
            self.is_valid = True
        except Exception as e:
            self.is_valid = False
            if raise_exception:
                raise e
            else:
                logging.warning(f"Agent Validation Error: {e}")
                logging.warning("You won't be able to run the Agent until the issues are handled manually.")
        return self.is_valid

    def run(
        self,
        data: Optional[Union[Dict, Text]] = None,
        query: Optional[Text] = None,
        session_id: Optional[Text] = None,
        history: Optional[List[Dict]] = None,
        name: Text = "model_process",
        timeout: float = 300,
        parameters: Dict = {},
        wait_time: float = 0.5,
        content: Optional[Union[Dict[Text, Text], List[Text]]] = None,
        max_tokens: int = 2048,
        max_iterations: int = 10,
        output_format: OutputFormat = OutputFormat.TEXT,
    ) -> AgentResponse:
        """Runs an agent call.

        Args:
            data (Optional[Union[Dict, Text]], optional): data to be processed by the agent. Defaults to None.
            query (Optional[Text], optional): query to be processed by the agent. Defaults to None.
            session_id (Optional[Text], optional): conversation Session ID. Defaults to None.
            history (Optional[List[Dict]], optional): chat history (in case session ID is None). Defaults to None.
            name (Text, optional): ID given to a call. Defaults to "model_process".
            timeout (float, optional): maximum amount of time to wait for result. Defaults to 300.
            parameters (Dict, optional): optional parameters to the model. Defaults to "{}".
            wait_time (float, optional): amount of time to wait between pooling requests. Defaults to 0.5.
            content (Union[Dict[Text, Text], List[Text]], optional): Content inputs to be processed according to the query. Defaults to None.
            max_tokens (int, optional): maximum number of tokens which can be generated by the agent. Defaults to 2048.
            max_iterations (int, optional): maximum number of iterations between the agent and the tools. Defaults to 10.
            output_format (ResponseFormat, optional): response format. Defaults to TEXT.

        Returns:
            AgentResponse: response data
        """
        start = time.time()

        # Reset error handler's retry count at the start of a new run
        self.error_handler.reset_retries()

        # Log agent start
        logging.info(f"Agent {self.id} started with query: {query or str(data)}")

        while True:
            try:
                # Call run_async to get polling URL
                response = self.run_async(
                    data=data,
                    query=query,
                    session_id=session_id,
                    history=history,
                    name=name,
                    parameters=parameters,
                    content=content,
                    max_tokens=max_tokens,
                    max_iterations=max_iterations,
                    output_format=output_format,
                )

                # Check if async call failed
                if response["status"] == ResponseStatus.FAILED:
                    end = time.time()
                    response["run_time"] = end - start

                    error = AgentExecutionError(
                        message=response.get("error", "Unspecified error in async call"),
                    )

                    # Handle the error through the error handler
                    error_result = self.error_handler.handle_error(
                        error,
                        operation="agent_execution",
                        context={"agent_id": self.id, "session_id": session_id, "name": name},
                    )

                    error_response = AgentResponse(
                        status=ResponseStatus.FAILED,
                        data=AgentResponseData(
                            input=data or query,
                            output=None,
                            session_id=session_id,
                            intermediate_steps=None,
                            execution_stats=None,
                        ),
                        error_message=str(error),
                    )

                    # Check if we should break the process for non-recoverable errors
                    if error_result.get("break_process", False):
                        logging.error(f"Breaking process due to non-recoverable error: {str(error)}")
                        return error_response

                    # Check if we should retry
                    if error_result.get("should_retry", False):
                        retry_after = error_result.get("retry_after", 1)
                        logging.warning(f"Retrying agent run after {retry_after}s")
                        time.sleep(retry_after)
                        continue

                    return error_response

                # Get polling URL and start polling
                poll_url = response["url"]
                end = time.time()

                # Poll for results with timeout
                try:
                    result = self.sync_poll(poll_url, name=name, timeout=timeout, wait_time=wait_time)

                    if result.status == ResponseStatus.FAILED:
                        if "iteration limit" in str(result.error_message).lower():
                            error = AgentIterationLimitError(
                                message=f"Agent stopped due to iteration limit ({max_iterations})",
                            )
                        else:
                            error = AgentExecutionError(
                                message=f"Agent stopped due to error: {str(result.error_message)}",
                            )

                        # Handle the error through the error handler
                        error_result = self.error_handler.handle_error(
                            error,
                            operation="agent_execution",
                            context={"agent_id": self.id, "session_id": session_id, "name": name},
                        )

                        error_response = AgentResponse(
                            status=ResponseStatus.FAILED,
                            data=AgentResponseData(
                                input=result.get("input", None),
                                output=result.get("output", None),
                                session_id=session_id,
                                intermediate_steps=None,
                                execution_stats=None,
                            ),
                            error_message=str(error),
                        )
                        return error_response

                    # Process successful result
                    result_data = result.get("data") or {}
                    output = result_data.get("output")
                    response = AgentResponse(
                        status=ResponseStatus.SUCCESS,
                        completed=True,
                        data=AgentResponseData(
                            input=result_data.get("input"),
                            output=output,
                            session_id=result_data.get("session_id"),
                            intermediate_steps=result_data.get("intermediate_steps"),
                            execution_stats=result_data.get("executionStats"),
                        ),
                        used_credits=result_data.get("usedCredits", 0.0),
                        run_time=result_data.get("runTime", end - start),
                    )

                    # Log successful completion
                    logging.info(f"Agent {self.id} completed successfully")

                    return response

                except Exception as e:
                    # Handle polling errors
                    end = time.time()

                    # Determine error type
                    if isinstance(e, TimeoutError):
                        error = AgentTimeoutError(
                            message=f"Agent execution timed out after {timeout} seconds",
                        )
                    else:
                        error = AgentExecutionError(
                            message=f"Error while polling for results: {str(e)}",
                        )

                    # Handle the error through the error handler
                    error_result = self.error_handler.handle_error(
                        error, operation="result_polling", context={"agent_id": self.id, "session_id": session_id, "name": name}
                    )

                    error_response = AgentResponse(
                        status=ResponseStatus.FAILED,
                        data=AgentResponseData(
                            input=data or query,
                            output=None,
                            session_id=session_id,
                            intermediate_steps=None,
                            execution_stats=None,
                        ),
                        error_message=str(error),
                    )

                    # Check if we should break the process for non-recoverable errors
                    if error_result.get("break_process", False):
                        logging.error(f"Breaking process due to non-recoverable error: {str(error)}")
                        return error_response

                    # Check if we should retry
                    if error_result.get("should_retry", False):
                        retry_after = error_result.get("retry_after", 1)
                        logging.warning(f"Retrying agent run after {retry_after}s")
                        time.sleep(retry_after)
                        continue

                    return error_response

            except Exception as e:
                # Handle unexpected errors
                end = time.time()

                error = AgentExecutionError(
                    message=f"Error in agent execution: {str(e)}",
                )

                # Handle the error through the error handler
                error_result = self.error_handler.handle_error(
                    error, operation="agent_execution", context={"agent_id": self.id, "session_id": session_id, "name": name}
                )

                error_response = AgentResponse(
                    status=ResponseStatus.FAILED,
                    data=AgentResponseData(
                        input=data or query,
                        output=None,
                        session_id=session_id,
                        intermediate_steps=None,
                        execution_stats=None,
                    ),
                    error_message=str(error),
                )

                # Check if we should break the process for non-recoverable errors
                if error_result.get("break_process", False):
                    logging.error(f"Breaking process due to non-recoverable error: {str(error)}")
                    return error_response

                # Check if we should retry
                if error_result.get("should_retry", False):
                    retry_after = error_result.get("retry_after", 1)
                    logging.warning(f"Retrying agent run after {retry_after}s")
                    time.sleep(retry_after)
                    continue

                return error_response

        # If we've exhausted all retries
        end = time.time()
        error = AgentExecutionError(
            message="Maximum retry attempts reached without success",
        )

        error_response = AgentResponse(
            status=ResponseStatus.FAILED,
            data=AgentResponseData(
                input=data or query,
                output=None,
                session_id=session_id,
                intermediate_steps=None,
                execution_stats=None,
            ),
            error_message=str(error),
        )
        return error_response

    def run_async(
        self,
        data: Optional[Union[Dict, Text]] = None,
        query: Optional[Text] = None,
        session_id: Optional[Text] = None,
        history: Optional[List[Dict]] = None,
        name: Text = "model_process",
        parameters: Dict = {},
        content: Optional[Union[Dict[Text, Text], List[Text]]] = None,
        max_tokens: int = 2048,
        max_iterations: int = 10,
        output_format: OutputFormat = OutputFormat.TEXT,
    ) -> AgentResponse:
        """Runs asynchronously an agent call.

        Args:
            data (Optional[Union[Dict, Text]], optional): data to be processed by the agent. Defaults to None.
            query (Optional[Text], optional): query to be processed by the agent. Defaults to None.
            session_id (Optional[Text], optional): conversation Session ID. Defaults to None.
            history (Optional[List[Dict]], optional): chat history (in case session ID is None). Defaults to None.
            name (Text, optional): ID given to a call. Defaults to "model_process".
            parameters (Dict, optional): optional parameters to the model. Defaults to "{}".
            content (Union[Dict[Text, Text], List[Text]], optional): Content inputs to be processed according to the query. Defaults to None.
            max_tokens (int, optional): maximum number of tokens which can be generated by the agent. Defaults to 2048.
            max_iterations (int, optional): maximum number of iterations between the agent and the tools. Defaults to 10.
            output_format (ResponseFormat, optional): response format. Defaults to TEXT.

        Returns:
            dict: polling URL in response
        """
        # Validate input parameters first
        if data is None and query is None:
            error = AgentError(
                message="Either 'data' or 'query' must be provided.",
            )
            logging.error(f"Agent {self.id}: {str(error)}")
            error_response = AgentResponse(
                status=ResponseStatus.FAILED,
                error_message=str(error),
            )
            return error_response

        # Prepare input data
        input_data = query
        if data is not None:
            if isinstance(data, dict):
                if "query" not in data:
                    error = AgentError(
                        message="When providing a dictionary, 'query' must be provided.",
                    )
                    logging.error(f"Agent {self.id}: {str(error)}")
                    error_response = AgentResponse(
                        status=ResponseStatus.FAILED,
                        error_message=str(error),
                    )
                    return error_response
                input_data = json.dumps(data)
            else:
                input_data = str(data)

        # Validate content if provided
        if content is not None:
            if isinstance(content, list):
                if len(content) > 3:
                    error = AgentError(
                        message="The maximum number of content inputs is 3.",
                    )
                    logging.error(f"Agent {self.id}: {str(error)}")
                    error_response = AgentResponse(
                        status=ResponseStatus.FAILED,
                        error_message=str(error),
                    )
                    return error_response
                if not isinstance(input_data, str):
                    error = AgentError(
                        message="When providing 'content', query must be text.",
                    )
                    logging.error(f"Agent {self.id}: {str(error)}")
                    error_response = AgentResponse(
                        status=ResponseStatus.FAILED,
                        error_message=str(error),
                    )
                    return error_response
            elif isinstance(content, dict):
                # Check if all keys in content are referenced in the query
                for key in content.keys():
                    if "{{" + key + "}}" not in input_data:
                        error = AgentError(
                            message=f"Key '{key}' not found in query.",
                        )
                        logging.error(f"Agent {self.id}: {str(error)}")
                        error_response = AgentResponse(
                            status=ResponseStatus.FAILED,
                            error_message=str(error),
                        )
                        return error_response

        # Check if agent is ready
        if not self.validate():
            error = AgentError(
                message="Agent validation failed",
            )
            logging.error(f"Agent {self.id}: {str(error)}")
            error_response = AgentResponse(
                status=ResponseStatus.FAILED,
                error_message=str(error),
            )
            return error_response

        # Process any template variables in the agent description
        try:
            input_data = process_variables(
                query=input_data,
                data=data if isinstance(data, dict) else {},
                parameters=parameters,
                agent_description=self.description,
            )
        except Exception as e:
            error = AgentError(
                message=str(e),
            )
            logging.error(f"Agent {self.id}: {str(error)}")
            error_response = AgentResponse(
                status=ResponseStatus.FAILED,
                error_message=str(error),
            )
            return error_response

        # Prepare request
        headers = {"x-api-key": self.api_key, "Content-Type": "application/json"}

        payload = {
            "id": self.id,
            "query": input_data,
            "sessionId": session_id,
            "history": history,
            "executionParams": {
                "maxTokens": (parameters["max_tokens"] if "max_tokens" in parameters else max_tokens),
                "maxIterations": (parameters["max_iterations"] if "max_iterations" in parameters else max_iterations),
                "outputFormat": output_format.value,
            },
        }

        payload.update(parameters)
        payload = json.dumps(payload)

        # Log LLM start
        logging.info(f"LLM started for agent {self.id}")

        try:
            # Make the HTTP request
            r = _request_with_retry("post", self.url, headers=headers, data=payload)

            # Process response based on status code
            if r.status_code >= 400:
                # Get error details from the response
                resp = r.json()
                error_message = resp.get("message", f"HTTP Error {r.status_code}")

                # Create appropriate error based on status code
                if r.status_code == 401:
                    # Unauthorized API key - non-recoverable
                    error = AuthenticationError(message=f"Unauthorized API key: {error_message}")
                elif 460 <= r.status_code < 470:
                    # Subscription errors - non-recoverable
                    error = ValidationError(message=f"Subscription expired: {error_message}")
                elif 470 <= r.status_code < 480:
                    # Billing errors - non-recoverable
                    error = BillingError(message=f"Insufficient credits: {error_message}")
                elif 480 <= r.status_code < 490:
                    # Supplier errors - recoverable with exponential backoff
                    error = SupplierError(message="Rate limit exceeded")
                elif 490 <= r.status_code < 500:
                    # Validation errors - non-recoverable
                    error = ValidationError(message=f"Invalid input parameters: {error_message}")
                elif r.status_code >= 500:
                    # Server errors - unspecified but recoverable
                    error = AgentExecutionError(
                        message=f"Unspecified error: {error_message}",
                    )
                else:
                    # Other errors
                    error = AgentError(message=f"HTTP Error {r.status_code}: {error_message}")

                # Process the error through the error handler
                error_result = self.error_handler.handle_error(
                    error, operation="api_request", context={"agent_id": self.id, "session_id": session_id, "name": name}
                )

                # Check if we should retry
                if error_result.get("should_retry", False):
                    retry_after = error_result.get("retry_after", 1)
                    time.sleep(retry_after)
                    return self.run_async(
                        data=data,
                        query=query,
                        session_id=session_id,
                        history=history,
                        name=name,
                        parameters=parameters,
                        content=content,
                        max_tokens=max_tokens,
                        max_iterations=max_iterations,
                        output_format=output_format,
                    )

                # No retry, return error response
                error_response = AgentResponse(
                    status=ResponseStatus.FAILED,
                    data=AgentResponseData(
                        input=data or query,
                        output=None,
                        session_id=session_id,
                        intermediate_steps=None,
                        execution_stats=None,
                    ),
                    error_message=str(error),
                )
                return error_response

            # Success response
            resp = r.json()
            poll_url = resp.get("data")

            # Log LLM end
            logging.info(f"LLM completed for agent {self.id}")

            return AgentResponse(
                status=ResponseStatus.IN_PROGRESS,
                url=poll_url,
                data=AgentResponseData(input=input_data),
                run_time=0.0,
                used_credits=0.0,
            )

        except Exception as e:
            # Unspecified error handling
            error = AgentExecutionError(message=f"Unspecified error in request for {name}: {str(e)}")

            # Process the error through the error handler
            error_result = self.error_handler.handle_error(
                error, operation="api_request", context={"agent_id": self.id, "session_id": session_id, "name": name}
            )

            # Check if we should retry
            if error_result.get("should_retry", False):
                retry_after = error_result.get("retry_after", 1)
                time.sleep(retry_after)
                return self.run_async(
                    data=data,
                    query=query,
                    session_id=session_id,
                    history=history,
                    name=name,
                    parameters=parameters,
                    content=content,
                    max_tokens=max_tokens,
                    max_iterations=max_iterations,
                    output_format=output_format,
                )

            # No retry, return error response
            error_response = AgentResponse(
                status=ResponseStatus.FAILED,
                data=AgentResponseData(
                    input=data or query,
                    output=None,
                    session_id=session_id,
                    intermediate_steps=None,
                    execution_stats=None,
                ),
                error_message=str(error),
            )
            logging.error(f"Agent Run Async: Error in running for {name}: {e}")
            return error_response

    def to_dict(self) -> Dict:
        return {
            "id": self.id,
            "name": self.name,
            "assets": [tool.to_dict() for tool in self.tools],
            "description": self.description,
            "role": self.instructions,
            "supplier": (self.supplier.value["code"] if isinstance(self.supplier, Supplier) else self.supplier),
            "version": self.version,
            "llmId": self.llm_id,
            "status": self.status.value,
            "tasks": [task.to_dict() for task in self.tasks],
        }

    def delete(self) -> None:
        """Delete Agent service"""
        try:
            url = urljoin(config.BACKEND_URL, f"sdk/agents/{self.id}")
            headers = {
                "x-api-key": config.TEAM_API_KEY,
                "Content-Type": "application/json",
            }
            logging.debug(f"Start service for DELETE Agent  - {url} - {headers}")
            r = _request_with_retry("delete", url, headers=headers)
            logging.debug(f"Result of request for DELETE Agent - {r.status_code}")
            if r.status_code != 200:
                raise Exception()
        except Exception:
            try:
                response_json = r.json()
                message = f"Agent Deletion Error (HTTP {r.status_code}): {response_json.get('message', '').strip('{{}}')}."
            except ValueError:
                message = f"Agent Deletion Error (HTTP {r.status_code}): There was an error in deleting the agent."
            logging.error(message)
            raise Exception(f"{message}")

    def update(self) -> None:
        """Update agent."""
        import warnings
        import inspect

        # Get the current call stack
        stack = inspect.stack()
        if len(stack) > 2 and stack[1].function != "save":
            warnings.warn(
                "update() is deprecated and will be removed in a future version. " "Please use save() instead.",
                DeprecationWarning,
                stacklevel=2,
            )
        from aixplain.factories.agent_factory.utils import build_agent

        self.validate(raise_exception=True)
        url = urljoin(config.BACKEND_URL, f"sdk/agents/{self.id}")
        headers = {"x-api-key": config.TEAM_API_KEY, "Content-Type": "application/json"}

        payload = self.to_dict()

        logging.debug(f"Start service for PUT Update Agent  - {url} - {headers} - {json.dumps(payload)}")
        resp = "No specified error."
        try:
            r = _request_with_retry("put", url, headers=headers, json=payload)
            resp = r.json()
        except Exception:
            raise Exception("Agent Update Error: Please contact the administrators.")

        if 200 <= r.status_code < 300:
            return build_agent(resp)
        else:
            error_msg = f"Agent Update Error (HTTP {r.status_code}): {resp}"
            raise Exception(error_msg)

    def save(self) -> None:
        """Save the Agent."""
        self.update()

    def deploy(self) -> None:
        assert self.status == AssetStatus.DRAFT, "Agent must be in draft status to be deployed."
        assert self.status != AssetStatus.ONBOARDED, "Agent is already deployed."
        self.status = AssetStatus.ONBOARDED
        self.update()

    def __repr__(self):
        return f"Agent(id={self.id}, name={self.name}, function={self.function})"
