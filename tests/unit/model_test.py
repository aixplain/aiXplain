...
def test_call_run_endpoint_sync():
    ...
    payload = {"data": "input_data"}
    ref_response = {"completed": True, "status": AssetStatus.SUCCESS, "data": "Hello"}
...
def test_success_poll():
    ...
    assert hyp_response["status"] == AssetStatus.SUCCESS
...
def test_failed_poll():
    ...
    assert response.status == AssetStatus.FAILED
