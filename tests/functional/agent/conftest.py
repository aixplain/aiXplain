...
# `pytest_collection_modifyitems` skip made the `agent` CI leg collect 35 tests,
# execute 0, and report green (ENG-3544). If an individual test is failing, skip
# or xfail *that test* with a ticket in the reason -- never the directory.


@pytest.fixture(autouse=True)
def _require_api_key():
    """Skip an agent functional test when there is no API key to run it with.
