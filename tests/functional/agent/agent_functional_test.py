...
def test_agent_with_utility_tool(delete_agents_and_team_agents, AgentFactory):
    ...
    inputs=[UtilityModelInput(name="string1", description="First string to concatenate", type=DataType.TEXT),
    ...
    agent = AgentFactory.create(
        ...
        tools=[AgentFactory.create_model_tool(model=vowel_remover_.id),
               AgentFactory.create_model_tool(model=concat_strings_.id),],
        llm_id="6646261c6eb563165658bbb1",
        )
    
    result_vowel = agent.run("Remove all the vowels in this string: string = Hello")
    result_concat_text = agent.run("Concat these strings: String1 = Hello; string2= World!.")
    
    assert "hll" in result_vowel['data']['output'].lower(), "The output should be or contain hll"
    assert "helloworld!" == result_concat_text['data']['output'].lower(), "The output should be helloworld!"
...
def test_agent_with_pipeline_tool(delete_agents_and_team_agents, AgentFactory):
    ...
    output_node = middle_node.use_output('data')
    pipeline.save()
    pipeline.deploy()
    ...
    pipeline_agent = AgentFactory.create(
                        ...
                        tools=[AgentFactory.create_pipeline_tool(pipeline=pipeline.id, description="You are a tool that responds users query with only 'Hello'."),],
                        llm_id="6646261c6eb563165658bbb1",
                        )
