...
def test_index_model_air_with_splitter(embedding_model, params_class):
    from aixplain.factories import IndexFactory
    from aixplain.modules.model.record import Record
    from uuid import uuid4
    ...
    params = params_class(name=f"Splitter Index {uuid4()}", description="Index for splitter", embedding_model=embedding_model)
    index_model = IndexFactory.create(params=params)
    index_model.upsert([Record(value="Berlin is the capital of Germany.", value_type="text", uri="", id="1", attributes={})], splitter=Splitter(split=True, split_by="word", split_length=1, split_overlap=0))
