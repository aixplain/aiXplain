...
def test_get_agent(client, agent_id):
    ...
    assert (
        agent.llm_id == "669a63646eb56306647e1091"
    ), f"Expected LLM ID '669a63646eb56306647e1091', got '{agent.llm_id}'"
    ...
    assert (
        agent.llm_id is not None
    ), "Agent LLM ID is None - agent may not be functional"
...
def test_slack_tool_integration_with_agent(client, slack_token):
    ...
    tool_response = slack_tool.run(
        {"channel": "#integrations-test", "text": test_message}
    )
