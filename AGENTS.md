...
Three modules are generated by `generate.py` and must never be hand-edited: `aixplain/v1/enums/generated_enums.py` (the v1 `Function`, `Supplier`, `Language` and `License` enums), `aixplain/v1/modules/pipeline/pipeline.py` (the v1 pipeline node classes) and `aixplain/v2/enums_include.py` (a re-export shim of the v1 enums that nothing in `v2` imports). The generator is v1-only: `v2`'s enums are hand-maintained in `aixplain/v2/enums.py`, `aixplain/v2/__init__.py` imports from there, and `import aixplain` loads no v1 module at all (asserted by `tests/unit/test_v1_deprecation_warning.py`). A fixture refresh therefore changes what v1 exposes, not what `Aixplain().Model.search(suppliers=...)` accepts. Known gap: the v2 `Supplier` lists seven members and lags the backend catalog, including the four suppliers the 2026-09-08 refresh added to v1 (`ALIBABA_CLOUD`, `ANTHROPIC`, `BYTEDANCE`, `OPENROUTER`).
...
Rendering reads only the committed fixtures in `tools/generator/fixtures/`, so it is deterministic and needs neither a backend nor a key; the `generator-drift` CI job runs `python generate.py render --check` and fails if the committed modules differ from a fresh render. `fetch` is the only networked step and is run by hand; it refuses a non-production `BACKEND_URL` unless `--allow-nonprod` is passed and records the host it captured from in `tools/generator/fixtures/provenance.json`, which a test asserts names production — see `tools/generator/fixtures/README.md` for the refresh workflow and why the fixtures are captured from production.
