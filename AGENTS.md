...
Three modules are generated by `generate.py` and must never be hand-edited: `aixplain/v1/enums/generated_enums.py`, `aixplain/v1/modules/pipeline/pipeline.py` and `aixplain/v2/enums_include.py`. The `Function`, `Supplier`, `Language` and `License` enums that `v2` uses all live in the first of these, so the generator is load-bearing for `v2` even though the files sit under `v1`.
